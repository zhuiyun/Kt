...
object KtHttpV2 {
    ...
    var baseUrl = "https://trendings.herokuapp.com" // 可改成任意url
    ...
    fun invoke(url: String, methed: Method, args: Array<Any>): Any? =
        methed.parameterAnnotations
            .takeIf { methed.parameterAnnotations.size == args.size }
            ?.mapIndexed { index, it -> Pair(it, args[index]) }
            ?.fold(url, ::parseUrl)
            ?.let { Request.Builder().url(it).build() }
            ?.let { okHttpClient.newCall(it).execute().body()?.string() }
            ?.let { gson.fromJson(it, methed.genericReturnType) }
